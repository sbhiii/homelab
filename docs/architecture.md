...
- **`route53:ListHostedZonesByName` is granted, and it is the one permission that cannot be scoped.** It takes no resource-level conditions, so the role can enumerate every hosted zone in the account. It was previously omitted, with the `ClusterIssuer` naming a zone ID instead, which kept the policy to two actions on one zone. That traded a wider policy for a narrower one, and it was given up on purpose: naming the zone ID meant publishing an AWS identifier in the gitops repository, and the goal there is that the repository contains none at all. The permission grants no read of record contents and no write, and this account holds one zone. See [`iac/aws/iam.tf`](../iac/aws/iam.tf).
